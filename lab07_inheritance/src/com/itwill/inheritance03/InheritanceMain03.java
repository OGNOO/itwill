...
public class InheritanceMain03 {
	...
	public static void main(String[] args) {
		...
		car3.drive(); // -> 다형성으로 선언되어 있더라도, override 되어 있는 메서드가 호출됨!
//		car3.charge(300); // -> casting 을 하지 않으면 SubType의 (실체 객체의 모든 메서드를 사용할 수는 없음
		((HybridCar) car3).charge(300); // -> casting 을 하면, SubType 의 모든 메서드를 사용할 수 있음.

		// SubType var = new SuperType(); -> 컴파일 오류
//		HybridCar car = new Car(100, 100); -> 문법 

		// 다형성 장점: 코드의 재사용성(re-usability)을 높여줌
		// (예) 배열 선언, 파라미터 선언 등에서
	...
	public static void test(Car car) {
		// instanceof 연산자: 객체가 어떤 타입인지를 반환하는 연산자
