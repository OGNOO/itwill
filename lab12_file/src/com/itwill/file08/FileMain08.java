...
//  이클립스 자바 프로그램을 실행(Run as Java application)할 때, 현재 작업 폴더 위치는 프로젝트 폴더
// System.getProperty(key): Java Runtime이 실행 중인 컴퓨터의 여러가지 시스템 정보들을 리턴

// 절대 경로(absolute path):
//  - 시스템의 루트(C:\, D:\, /)부터 파일 또는 폴더가 있는 위치까지를 찾아가는 방식
//  - (Windows 예) C:\workspaces\lab-java\Java13_File\data\hello.txt
//  - (Linux, MacOS 예) /users/itwill/documents/hello.txt
// 상대 경로(relative path):
//  - 현재 작업 폴더를 기준으로 파일 또는 폴더 위치를 상대적으로 찾아가는 방식
...
public class FileMain08 {
	...
	public static void main(String[] args) {
		// 이클립스에서 자바 프로그램이 실행되는 현재 작업 폴더 디렉토리(CWD)
		String cwd = System.getProperty("user.dir");
		System.out.println("CWD: " + cwd);

		String path = "C:\\study\\workspace\\itwill\\lab12_file"; // Windows 절대 경로 표현법
		System.out.println(path);

		// String path2= "data\\hello.txt"; // Windows 상대 경로 표현법
		String path2 = "data" + File.separator + "hello.txt"; // File.separator -> OS에 무관한 상대 경로 표현법
		...
		System.out.println("크기 여부: " + f.length()); // -> 바이트(byte) 단위
