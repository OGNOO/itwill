...
//     - 객체를 생성한 후에, 그 참조 변수(인스턴스)로 접근해서 사용하는 필드
//   (2) 정적(static) 필드: static 으로 선언한 필드
//     - 프로그램이 시작될 때, 클래스 로더에 의해서 메모리에 미리 로딩되는 변수
//     - 객체를 생성하지 않고, 클래스 이름을 접두사로 사용하는 필드
// 2. 지역 변수: 메서드 안에서 선언한 변수. 접근 수식어를 사용할 수 없음
//   - 선언된 위치에서부터 변수가 포함된 블록({...})이 끝날 때까지만 사용 가능
//   - 메서드의 파라미터도 지역 변수의 한 종류
//   - 지역 변수에는 static 을 사용 할 수 없음!
//   - 접근 수식어(private, protected, public)를 사용할 수 없음!
// 
// 내부 클래스/인터페이스(inner class/interface): 
//   다른 클래스/인터페이스 안에서 선언하는 클래스/인터페이스
...
//   (1) 인스턴스 내부 클래스: static 이 아닌 멤버 내부 클래스
//     - 외부 클래스의 객체를 먼저 생성하고 난 후에, 그 참조 변수로 참조해서 생성하고 사용할 수 있는 멤버 내부 클래스
//   (2) static 내부 클래스: static 으로 선언한 멤버 내부 클래스
//     - 외부 클래스의 객체 생성 여부와 상관없이 사용할 수 있는 멤버 내부 클래스
//     - 중첩 클래스(nested class)
//     - 멤버 내부 인터페이스는 static 이어야만 함. static 은 생략
// 2. 지역 (내부) 클래스(local class): 메서드 안에서 선언하는 클래스
//   - 선언된 메서드의 블록 안에서만 객체를 생성하고 사용할 수 있는 클래스
