...
public class InnerMain03 {
	...
	public static void main(String[] args) {
		...
		class CloseOnButtonListener implements OnClickListener {
			@Override
			public void onClick() {
		...
		// 3. OnClickListener 를 구현하는 클래스 타입 객체 생성하고 버튼에 리스너 설정
		btnClose.setOnClickListener(new CloseOnButtonListener());

		// 4. 닫기 버튼 클릭
		btnClose.click();

		// ----- 익명 클래스를 사용한 버튼 객체 생성 및 리스너 설정
		// 1. 버튼 생성
		Button btnAdd = new Button("Add");

		// 2. 리스터 구현 객체 생성
		OnClickListener addListener = new OpenButtonListener() {
			@Override
			public void onClick() {
		...
		// ----- 익명 클래스 2
		Button btnCanncel = new Button("취소");
		btnCanncel.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick() {
