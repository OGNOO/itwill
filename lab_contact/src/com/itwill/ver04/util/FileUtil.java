...
// 객체를 생성하지 못하도록 작성. 모든 메서드는 public static 으로 작성.
public class FileUtil {
	...
	/**
	 * 연락처 데이터 파일을 저장하는 폴더가 생성되어 있지 않으면, 폴더를 새로 생성하고 File 타입 객체를 리턴. 데이터 폴더가 이미
	 * 존재하면, 그 폴더에 File 객체를 리턴.
	 * 
	 * @return File 객체.
	 */
...
//			System.out.println("데이터 폴더가 이미 있습니다...");
	...
	public static List<Contact> readDataFromFile() {
		...
		try (FileInputStream fis = new FileInputStream(file);
			...
			for (Object object : result) {
				if (object instanceof Contact && list != null) {
	...
	/**
	 * 연락처 파일이 존재하면, 파일의 내용을 읽어서 리스트를 리턴. 연락처 파일이 없으면, 빈(원소가 없는) 리스트를 리턴
	 * 
	 * @return 연락처 리스트(List<Contact>)
	 */
