...
public interface ContactDao {
	/**
	 * 새로운 연락처 정보(이름, 전화번호, 이메일)을 리스트에 저장. 바뀐 리스트의 내용을 파일에 저장.
	 * 
	 ...
	 * @return 아규먼트 index 가 리스트의 인덱스 범위 안에 있으면 Contact 타입 객체를 리턴. index가 리스트 범위 밖에
	 *         있거나, 해당 인덱스의 리스트 원소가 null 이면 null 을 리턴.
	 */
	Contact read(int index);

	/**
	 * 연락처 정보 업데이트 기능. 리스트의 특정 인덱스의 연락처 정보(이름, 전화번호, 이메일)을 업데이트. 바뀐 리스트의 내용을 파일에 저장.
	 * 
	...
	/**
	 * 연락처 삭제 기능. 해당 인덱스의 연락처를 리스트에서 삭제 바뀐 리스트의 내용을 파일에 저장.
	 * 
	...
	/**
	 * 연락처 검색 기능. 검색어가 이름, 전화번호, 이메일에 포함된 연락처 정보를 리스트에 넣어서 리턴
	 * 
